  ...
  return (
    ...
    >
      <div className="pt-10 hover:text-slate-100 text-slate-600">
        <Link href="/" className="flex  items-center px-4 py-3 rounded-xl text-slate-700 hover:bg-main  transition duration-150 cursor-pointer">
          <Image
            width={40}
            height={40}
            src="/avatar.png"
        ...
        <ul>
          {dashboardItems.map((item, index) => (
            <li key={item.title}>
              <Link
                href={item.link}
                className={`flex mb-4 items-center px-4 py-3 rounded-xl text-slate-700 hover:bg-violet-500 hover:text-white transition duration-150 cursor-pointer ${activePath === item.title.toLowerCase() && "bg-violet-600 text-white" }`}
              >
                <i>
